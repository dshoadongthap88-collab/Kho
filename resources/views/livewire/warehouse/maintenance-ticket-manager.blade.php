                    ...
                    @forelse ($tickets as $ticket)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">
                                {{ $ticket->asset->name ?? 'N/A' }}<br>
                                <span class="text-xs font-normal text-gray-500">{{ $ticket->asset->asset_code ?? '' }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-700">{{ $ticket->maintenance_rule_id ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-600 text-right">{{ number_format($ticket->maintenance_odo) }}</td>

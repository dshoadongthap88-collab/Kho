            ...
            <div class="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                @forelse($assets as $asset)
                    <button wire:click="selectAsset({{ $asset->id }})" 
                            class="w-full text-left px-3 py-2 rounded-lg transition-colors border {{ $selectedAssetId == $asset->id ? 'bg-sky-50 border-sky-300 shadow-sm' : 'border-transparent hover:bg-slate-50' }}">
                        <div class="font-bold text-slate-800 text-sm uppercase">{{ $asset->name }}</div>
                        <div class="text-xs text-slate-500 font-mono">{{ $asset->equipment_code ?: 'N/A' }}</div>
                    </button>
                @empty
                ...
                <div class="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                    @forelse($boms as $bom)
                        <div class="flex items-center gap-2">
                            <button wire:click="selectBom({{ $bom->id }})" 
                                    class="flex-1 text-left px-3 py-2 rounded-lg transition-colors border {{ $selectedBomId == $bom->id ? 'bg-indigo-50 border-indigo-300 shadow-sm' : 'border-transparent hover:bg-slate-50' }}">
                                <div class="font-bold text-slate-800 text-sm">{{ $bom->maintenance_level }}</div>
                                <div class="text-[10px] text-slate-500 uppercase">{{ $bom->bom_code }}</div>
                            </button>
                            <!-- Sửa / Xóa BOM -->
                            <div class="flex flex-col gap-1">
                                <button wire:click="openBomModal({{ $bom->id }})" class="text-blue-500 hover:text-blue-700 bg-blue-50 p-1 rounded"><svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg></button>
                                <button onclick="if(confirm('Bạn có chắc muốn xóa Mức bảo dưỡng này và toàn bộ vật tư bên trong?')) @this.deleteBom({{ $bom->id }})" class="text-red-500 hover:text-red-700 bg-red-50 p-1 rounded"><svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                            </div>
                        </div>
                    @empty
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead class="bg-slate-100 text-[11px] uppercase font-bold text-slate-600 sticky top-0">
                                    <tr>
                                        <th class="px-3 py-2 border-b border-slate-200 w-12 text-center">
                                            <input type="checkbox" wire:model.live="selectAllProducts" wire:click="toggleSelectAllProducts({{ json_encode($allProductIdsOnPage) }})" class="rounded text-indigo-600 focus:ring-indigo-500">
                                        </th>
                                        <th class="px-3 py-2 border-b border-slate-200">Mã VT</th>
                                        <th class="px-3 py-2 border-b border-slate-200">Tên vật tư</th>
                                        <th class="px-3 py-2 border-b border-slate-200 text-center">ĐVT</th>
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @forelse($products as $prod)
                                        <tr class="hover:bg-indigo-50 transition-colors cursor-pointer" onclick="document.getElementById('chk-{{$prod->id}}').click()">
                                            <td class="px-3 py-2 text-center" onclick="event.stopPropagation()">
                                                <input id="chk-{{$prod->id}}" type="checkbox" value="{{ $prod->id }}" wire:model="selectedProductIds" class="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4">
                                            </td>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t">
                        <button type="button" wire:click="addSelectedProductsToBom" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-bold text-white hover:bg-green-700 sm:ml-3 sm:w-auto sm:text-sm">
                            Thêm {{ count($selectedProductIds) }} vật tư vào BOM
                        </button>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-2 sm:pb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <select wire:model="copyFromBomId" class="w-full border-gray-300 rounded-md shadow-sm p-2 border bg-white">
                                ...
                                @foreach($otherBoms as $ob)
                                    <option value="{{ $ob->id }}">{{ $ob->maintenance_level }} (Chu kỳ: {{ $ob->cycle }})</option>
                                @endforeach
                            </select>
                            @error('copyFromBomId') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">

                    ...
                    @if(count($lowStockProducts ?? []) > 0)
                        ...
                        <ul class="text-sm text-amber-900 space-y-2 max-h-[500px] overflow-y-auto px-2 print:max-h-none print:overflow-visible">
                            @foreach($lowStockProducts ?? [] as $index => $item)
                                ...
                                @endphp
                                <li :class="{ 'print:hidden': selectedWarningLow.length > 0 && !selectedWarningLow.includes('{{ $index }}') }" class="flex items-start gap-3 hover:bg-amber-100 p-2 rounded transition-colors">
                                    <input type="checkbox" value="{{ $index }}" x-model="selectedWarningLow" class="row-cb-low mt-1 rounded border-gray-300 text-amber-600 focus:ring-amber-500 print:hidden">
                                    <div>
                    ...
                    @if(count($highStockProducts ?? []) > 0)
                        ...
                        <ul class="text-sm text-rose-900 space-y-2 max-h-[500px] overflow-y-auto px-2 print:max-h-none print:overflow-visible">
                            @foreach($highStockProducts ?? [] as $index => $item)
                                ...
                                @endphp
                                <li :class="{ 'print:hidden': selectedWarningHigh.length > 0 && !selectedWarningHigh.includes('{{ $index }}') }" class="flex items-start gap-3 hover:bg-rose-100 p-2 rounded transition-colors">
                                    <input type="checkbox" value="{{ $index }}" x-model="selectedWarningHigh" class="row-cb-high mt-1 rounded border-gray-300 text-rose-600 focus:ring-rose-500 print:hidden">
                                    <div>
                                        <b class="text-base">{{ $item->name }}</b> <span class="text-gray-600">({{ $item->code }})</span>

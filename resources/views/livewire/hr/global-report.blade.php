<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-2 mb-10">
        <div class="bg-gradient-to-br from-indigo-500 to-blue-600 rounded-2xl p-2 text-white shadow-lg relative overflow-hidden">
            ...
            <div class="text-5xl font-black relative z-10">{{ $projects->count() }}</div>
        </div>
        <div class="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-2 text-white shadow-lg relative overflow-hidden">
            <div class="absolute right-0 top-0 opacity-10 transform translate-x-4 -translate-y-4">
            ...
            <div class="text-5xl font-black relative z-10">{{ $totalUsers }}</div>
        </div>
        <div class="bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl p-2 text-white shadow-lg relative overflow-hidden">
            <div class="absolute right-0 top-0 opacity-10 transform translate-x-4 -translate-y-4">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-2">
        ...
        @php $pStats = $stats[$project->id] ?? []; @endphp
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
            <!-- Header của Ngôi nhà -->
            <div class="bg-slate-50 border-b border-slate-200 p-2 flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-slate-500 font-medium text-xs mt-0.5">Mã: <span class="text-indigo-600">{{ $project->code ?? "N/A" }}</span> &bull; <span class="text-emerald-600">{{ $pStats["users"] ?? 0 }} NS</span></p>
                    </div>
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <div class="bg-orange-50 border border-orange-200 rounded-lg px-2 py-1.5 flex items-center gap-2" title="Sắp hết tồn kho">
                        ...
                        <div class="text-lg font-black text-orange-600 leading-none">{{ $pStats["low_stock_count"] ?? 0 }}</div>
                    </div>
                    ...
                    <div class="bg-red-50 border border-red-200 rounded-lg px-2 py-1.5 flex items-center gap-2" title="Chưa dùng > 300 ngày">
                        ...
                        <div class="text-lg font-black text-red-600 leading-none">{{ $pStats["obsolete_stock_count"] ?? 0 }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-0 divide-y md:divide-y-0 md:divide-x divide-slate-100 flex-1">
                ...
                <div class="p-2">
                    ...
                    @if(isset($pStats["top_assets"]) && count($pStats["top_assets"]) > 0)
                        <div class="space-y-3">
                            @foreach($pStats["top_assets"] as $index => $asset)
                            <div class="group flex items-center justify-between p-3 rounded-xl border border-slate-100 hover:border-blue-200 hover:bg-blue-50 transition-colors">
                                ...
                                <div class="text-sm font-bold text-slate-600 group-hover:text-blue-700 bg-slate-100 group-hover:bg-blue-200 px-3 py-1 rounded-full">
                                    {{ number_format($asset->usage_count) }} lần xuất
                                </div>
                            </div>
                            @endforeach
                        </div>
                ...
                <div class="p-2">
                    ...
                    @if(isset($pStats["top_materials"]) && count($pStats["top_materials"]) > 0)
                        <div class="space-y-2">
                            @foreach($pStats["top_materials"] as $index => $mat)
                            <div class="group flex items-center justify-between p-2 rounded-lg border border-slate-100 hover:border-purple-200 hover:bg-purple-50 transition-colors">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-slate-500 font-mono mt-0.5">Mã: {{ $mat->code }}</div>
                                    </div>
                                </div>

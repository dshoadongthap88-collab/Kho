                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($plans as $plan)
                        ...
                        <tr class="{{ $rowClass }} transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{{ $plan->plan_code }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-800">{{ $plan->category }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-right text-gray-900">{{ number_format($currentVal) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-right text-indigo-600">{{ number_format($plan->maintenance_odo) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="px-2.5 py-1 inline-flex text-xs font-bold rounded-full border {{ $badgeClass }}">{{ $badgeText }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button wire:click="markAsCompleted({{ $plan->id }})" class="text-green-700 hover:text-green-900 mx-2 bg-green-100 border border-green-200 px-3 py-1.5 rounded-lg shadow-sm" title="Đã hoàn thành">Đã bảo dưỡng ✅</button>
                            </td>
                        </tr>
                    @empty
                        <tr>

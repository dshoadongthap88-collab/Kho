    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
        ...
        <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
            <button wire:click="printSelected" class="flex items-center justify-center gap-1.5 bg-slate-700 hover:bg-slate-800 text-white px-3 py-1.5 rounded-lg text-sm font-medium shadow transition">
                ...
                In phiếu
                @if(count($selected) > 0)
                    <span class="bg-white text-slate-800 text-[10px] font-bold px-1.5 py-0.5 rounded-full ml-1">{{ count($selected) }}</span>
                @endif
            </button>
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse($plans as $plan)
                        ...
                        <tr class="hover:bg-slate-50 transition border-b border-slate-100">
                            ...
                            <td class="px-2 py-1.5 whitespace-nowrap text-xs">{{ $plan->created_at->format('d/m/Y') }}</td>
                            <td class="px-2 py-1.5 text-sm font-medium text-slate-900 leading-tight">
                                <div class="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded text-[10px] font-bold inline-block mb-0.5">{{ $plan->product?->code ?? 'N/A' }}</div><br>
                                <span class="line-clamp-2 max-w-[180px]" title="{{ $plan->product?->name ?? 'Vật tư đã bị xóa' }}">{{ $plan->product?->name ?? 'Vật tư đã bị xóa' }}</span>
                            </td>
                            ...
                            <td class="px-2 py-1.5 text-right font-bold text-rose-600 text-sm whitespace-nowrap">{{ number_format($missing, 0) }}</td>
                            ...
                            <td class="px-2 py-1.5 text-center whitespace-nowrap">
                                @if($plan->status !== 'completed')
                                    <select wire:change="updateUrgency({{ $plan->id }}, $event.target.value)" class="p-1 text-[11px] h-7 border-slate-300 rounded focus:ring-sky-500 focus:border-sky-500 {{ $plan->urgency === 'urgent' ? 'text-rose-600 font-bold bg-rose-50' : 'text-slate-600' }}">
                                ...
                                @else
                                    <span class="text-[11px] px-1.5 py-0.5 rounded {{ $plan->urgency === 'urgent' ? 'text-rose-600 font-bold bg-rose-50' : 'text-slate-600' }}">
                            ...
                            <td class="px-2 py-1.5 text-right space-x-1 whitespace-nowrap">
                                @if($plan->status === 'pending')
                                    <button wire:click="placeOrder({{ $plan->id }})" class="px-1.5 py-1 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded text-[11px] font-bold transition">Đặt hàng</button>
                                @endif
                                
                                @if($plan->status !== 'completed')
                                    <button wire:click="openUpdateModal({{ $plan->id }})" class="px-1.5 py-1 bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white rounded text-[11px] font-bold transition">Nhận</button>
                                @endif

                                <button wire:click="delete({{ $plan->id }})" wire:confirm="Bạn có chắc chắn muốn xóa?" class="px-1.5 py-1 text-rose-600 hover:bg-rose-50 rounded text-[11px] transition">Xóa</button>
                            </td>
                        </tr>
                    @empty
                        <tr>

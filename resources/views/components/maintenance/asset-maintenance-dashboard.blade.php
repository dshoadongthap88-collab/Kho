            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($assets as $asset)
                    <tr class="hover:bg-slate-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{{ $asset['asset_code'] }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-600">{{ $asset['name'] }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ number_format($asset['current_odo'], 1) }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $asset['maintenance_cycle'] }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold {{ $asset['is_overdue'] ? 'text-red-600' : ($asset['is_warning'] ? 'text-yellow-600' : 'text-green-600') }}">

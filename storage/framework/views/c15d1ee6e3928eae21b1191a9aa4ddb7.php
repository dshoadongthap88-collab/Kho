            ...
            <table class="w-full text-sm text-left">
                <thead class="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
                    <tr>
                        ...
                        <th class="px-2 py-2 text-center">Trạng Thái Mới</th>
                        <th class="px-2 py-2 text-right">SL Đã Giao Mới</th>
                        <th class="px-2 py-2">Ghi Chú & Nguồn Thay Đổi</th>
                        <th class="px-2 py-2">Người Thực Hiện</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $histories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $history): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr class="hover:bg-slate-50 transition">
                            <td class="px-2 py-1.5 whitespace-nowrap text-slate-500"><?php echo e($history->created_at->format('d/m/Y H:i:s')); ?></td>
                            <td class="px-2 py-1.5 font-medium text-slate-900">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($history->purchasePlan): ?>
                                    <span class="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-xs mr-1"><?php echo e($history->purchasePlan->product?->code ?? 'N/A'); ?></span>
                                    <?php echo e($history->purchasePlan->product?->name ?? 'Vật tư đã bị xóa'); ?>

                                <?php else: ?>
                                    <span class="text-rose-500 italic">Dữ liệu đã bị xóa</span>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </td>
                            <td class="px-2 py-1.5 text-right font-bold text-sky-600">
                            ...
                            <td class="px-2 py-1.5 text-center">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($history->new_status === 'pending'): ?>
                                    <span class="px-1.5 py-1 text-[11px] bg-slate-100 text-slate-600 rounded-full text-xs font-bold">Đề xuất</span>
                                <?php elseif($history->new_status === 'ordered'): ?>
                                    <span class="px-1.5 py-1 text-[11px] bg-blue-100 text-blue-600 rounded-full text-xs font-bold">Đã đặt</span>
                                <?php elseif($history->new_status === 'unreceived'): ?>
                                    <span class="px-1.5 py-1 text-[11px] bg-rose-100 text-rose-600 rounded-full text-xs font-bold">Chưa giao</span>
                                <?php elseif($history->new_status === 'partial'): ?>
                                    <span class="px-1.5 py-1 text-[11px] bg-amber-100 text-amber-600 rounded-full text-xs font-bold">Giao thiếu</span>
                                <?php elseif($history->new_status === 'completed'): ?>
                                    <span class="px-1.5 py-1 text-[11px] bg-emerald-100 text-emerald-600 rounded-full text-xs font-bold">Đủ hàng</span>
                                <?php else: ?>
                                    -
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </td>
                            <td class="px-2 py-1.5 text-right font-bold text-emerald-600">
                            ...
                            <td class="px-2 py-1.5 text-slate-600"><?php echo e($history->notes); ?></td>

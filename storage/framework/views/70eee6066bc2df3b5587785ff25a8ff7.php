            ...
            <tbody class="divide-y divide-slate-100 text-sm">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $debts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $report): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                    ...
                    <tr <?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::$currentLoop['key'] = 'debt-'.e($report->id).''; ?>wire:key="debt-<?php echo e($report->id); ?>" class="hover:bg-slate-50/80 transition group <?php echo e($isOverdue ? 'bg-red-50/30' : ''); ?> <?php echo e(in_array((string)$report->id, $selectedIds) ? 'bg-blue-50/30 is-selected' : ''); ?> print-row">
                        ...
                        <!-- Còn lại -->
                        <td class="px-4 py-3 text-right font-bold <?php echo e($remaining > 0 ? 'text-red-600' : 'text-slate-400'); ?> print:text-black">
                        ...
                        <td class="px-4 py-3 text-center print:hidden">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($remaining > 0): ?>
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button wire:click="openPayModal(<?php echo e($report->id); ?>)" class="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 flex items-center justify-center rounded transition" title="Thu tiền">
                                        💰
                                    </button>
                                    <button wire:confirm="Khách hàng đã trả hết nợ hóa đơn này?" wire:click="markAsFullyPaid(<?php echo e($report->id); ?>)" class="bg-emerald-500 hover:bg-emerald-600 text-white px-2 py-1 flex items-center justify-center rounded transition" title="Xong nợ">
                                        ✅
                                    </button>
                                    <button wire:click="openEditModal(<?php echo e($report->id); ?>)" class="bg-slate-200 hover:bg-slate-300 text-slate-700 px-2 py-1 flex items-center justify-center rounded transition" title="Chỉnh sửa">
                                        📝
                                    </button>
                                    <button wire:confirm="Xác nhận xóa bản ghi công nợ này?" wire:click="delete(<?php echo e($report->id); ?>)" class="bg-rose-100 hover:bg-rose-200 text-rose-600 px-2 py-1 flex items-center justify-center rounded transition" title="Xóa">
                                        🗑️
                                    </button>
                                </div>
                            <?php else: ?>

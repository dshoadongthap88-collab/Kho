                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($trackingList as $item)
                        <tr class="hover:bg-slate-50 transition-colors {{ $item['color'] === 'red' ? 'bg-red-50/30' : ($item['color'] === 'yellow' ? 'bg-yellow-50/30' : '') }}">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600 font-bold text-right">{{ number_format($item['cycle']) }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 text-right">{{ number_format($item['last_hours']) }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-900 text-right">{{ number_format($item['current_hours']) }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-bold text-blue-600 text-right">{{ number_format($item['target_hours']) }}</td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-center">
                                @if($item['color'] === 'red')
                                    <span class="px-2 py-1 inline-flex text-xs leading-4 font-extrabold rounded-md bg-red-100 text-red-800 border border-red-200 uppercase">{{ $item['status'] }}</span>
                                @elseif($item['color'] === 'yellow')
                                    <span class="px-2 py-1 inline-flex text-xs leading-4 font-bold rounded-md bg-yellow-100 text-yellow-800 border border-yellow-200 uppercase">{{ $item['status'] }}</span>
                                @else
                                    <span class="px-2 py-1 inline-flex text-xs leading-4 font-bold rounded-md bg-green-100 text-green-800 border border-green-200 uppercase">{{ $item['status'] }}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-center text-sm font-medium">
                                @if($item['status'] === 'ĐẾN HẠN' || $item['status'] === 'SẮP ĐẾN')
                                    <a href="{{ route('warehouse.maintenance-plans') }}?create=1&asset={{ $item['asset_id'] }}" class="text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-1.5 rounded-md shadow-sm transition text-xs font-bold inline-block">Lên Kế Hoạch</a>
                                @else

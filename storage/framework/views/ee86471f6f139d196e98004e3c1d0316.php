    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-4 no-print relative z-10 bg-white p-3 rounded-xl shadow-sm border border-slate-200">
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex gap-1 bg-slate-100 p-1 rounded-lg">
                <button wire:click="$set('filterMode', 'all')" class="px-3 py-1 text-[10px] font-black uppercase rounded transition-all <?php echo e($filterMode === 'all' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'); ?>">Tất cả</button>
                <button wire:click="$set('filterMode', 'expiring')" class="px-3 py-1 text-[10px] font-black uppercase rounded transition-all <?php echo e($filterMode === 'expiring' ? 'bg-rose-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'); ?>">Sắp hết hạn</button>
                <button wire:click="$set('filterMode', 'low_stock')" class="px-3 py-1 text-[10px] font-black uppercase rounded transition-all <?php echo e($filterMode === 'low_stock' ? 'bg-amber-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'); ?>">Sắp hết tồn</button>
            </div>
        ...
        <div class="flex items-center gap-2">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(count($selectedProducts) > 0): ?>
                <div <?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::$currentLoop['key'] = 'bulk-actions-container'; ?>wire:key="bulk-actions-container" class="flex items-center gap-1.5 pr-2 border-r border-slate-300 mr-1 py-1">
                    <span class="text-[10px] font-black text-indigo-700 bg-indigo-50 px-2 py-1 rounded border border-indigo-100">CHỌN: <?php echo e(count($selectedProducts)); ?></span>
                ...
                <tr class="bg-slate-800 border-b border-slate-700 text-white uppercase text-[11px] font-black tracking-widest">
                    <th class="px-6 py-4 w-10 no-print text-center">
                        <input type="checkbox" wire:click="toggleSelectAll([<?php echo e(implode(',', $allProductIdsOnPage)); ?>])" 
                               <?php echo e(count($selectedProducts) === count($allProductIdsOnPage) && count($allProductIdsOnPage) > 0 ? 'checked' : ''); ?>
            ...
            <tbody class="divide-y divide-gray-100">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                    <tr class="hover:bg-indigo-50/30 transition-all border-b border-slate-100 
                        <?php echo e($product->is_expiring_soon ? 'bg-rose-50/50' : ($product->is_low_stock ? 'bg-amber-50/50' : '')); ?>

                        <?php echo e(in_array($product->id, $selectedProducts) ? 'bg-indigo-50' : ''); ?>">
                        <td class="px-6 py-4 no-print text-center">
                            <input type="checkbox" wire:model.live="selectedProducts" value="<?php echo e($product->id); ?>" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                        </td>
                        <td class="px-3 py-4 text-center text-xs font-bold text-slate-500">
                            <?php echo e($index + 1 + ($products->currentPage() - 1) * $products->perPage()); ?>
                        ...
                        <td class="px-4 py-3 text-center">
                            ...
                            <?php else: ?>
                                <button wire:click="openModal(<?php echo e($product->id); ?>)" class="text-[10px] font-black text-indigo-600 hover:text-indigo-800 uppercase underline decoration-2 underline-offset-4 cursor-pointer">
                                    Tải ảnh
                                </button>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </td>
                        <td class="px-4 py-4 font-black text-[13px] text-indigo-600 tracking-tight"><?php echo e($product->code); ?></td>
                        <td class="px-4 py-4">
                            <div class="font-black text-slate-900 text-[13px] uppercase tracking-tight"><?php echo e($product->name); ?></div>
                            <div class="text-[10px] text-slate-400 font-bold mt-0.5">VỊ TRÍ: <?php echo e($product->inventory?->warehouse_location ?? ($product->location ?: 'CHƯA XÁC ĐỊNH')); ?></div>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($product->brand): ?>
                                <div class="text-[10px] text-slate-500 font-bold">HÃNG SX: <?php echo e($product->brand); ?></div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </td>
                        <td class="px-4 py-4 text-center">
                            <span class="px-3 py-1.5 rounded-xl text-[13px] font-black shadow-sm border
                                <?php echo e($product->is_low_stock ? 'bg-rose-600 text-white border-rose-700' : (($product->inventory?->quantity ?? 0) > 0 ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-slate-100 text-slate-400 border-slate-200')); ?>">
                        ...
                        <td class="px-4 py-4 text-[12px] font-bold text-slate-700 uppercase"><?php echo e($product->unit); ?></td>
                        <td class="px-4 py-4 text-center text-[12px] font-bold text-slate-600">
                            <?php echo e(number_format($product->min_stock)); ?>
                        ...
                        <td class="px-4 py-4 text-right no-print">
                            <div class="flex items-center justify-end gap-1">
                                <button wire:click="openModal(<?php echo e($product->id); ?>)" class="p-2 text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all cursor-pointer" title="Sửa">📝</button>
                                <button wire:confirm="Xoá nguyên vật liệu này?" wire:click="delete(<?php echo e($product->id); ?>)" class="p-2 text-rose-300 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all cursor-pointer" title="Xoá">🗑️</button>
                            </div>
                        </td>
                    </tr>
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-middle bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4"><?php echo e($isEdit ? 'Chỉnh sửa NVL' : 'Thêm NVL mới'); ?></h3>
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <div class="col-span-1">
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['box_spec'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-red-500 text-xs"><?php echo e($message); ?></span> <?php unset($message);
...
<?php /**PATH D:\Project\resources\views\livewire\warehouse\material-catalog.blade.php ENDPATH**/ ?>
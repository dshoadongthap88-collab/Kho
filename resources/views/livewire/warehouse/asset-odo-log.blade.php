                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($logs as $log)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $log->reading_date->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 text-sm text-indigo-600 font-semibold">{{ $log->asset->asset_code ?? '' }} - {{ $log->asset->name ?? '' }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-green-600 text-right">+{{ number_format($log->odo_diff) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">{{ number_format($log->old_hours) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">{{ number_format($log->new_hours) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-green-600 text-right">+{{ number_format($log->hours_diff) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">{{ $log->updated_by }}</td>
                                ...
                                <div class="p-4 bg-slate-50 rounded-lg border border-slate-200">
                                    ...
                                    <div class="grid grid-cols-3 gap-4">
                                        ...
                                        <div>
                                            ...
                                            @error('new_hours') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                        </div>
                                        <div>

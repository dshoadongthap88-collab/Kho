<div class="px-4">
    ...
    <div class="bg-white p-2 rounded-xl shadow-sm border border-gray-100 mb-6 flex flex-wrap gap-2 items-center justify-between">
        ...
        <div class="flex gap-2">
            <button wire:click="$set('statusFilter', '')" class="px-3 py-1.5 text-sm font-medium rounded-lg <?php echo e($statusFilter === '' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-50'); ?>">Tất cả</button>
            <button wire:click="$set('statusFilter', 'pending')" class="px-3 py-1.5 text-sm font-medium rounded-lg <?php echo e($statusFilter === 'pending' ? 'bg-yellow-50 text-yellow-700' : 'text-gray-600 hover:bg-gray-50'); ?>">Đã lập</button>
            <button wire:click="$set('statusFilter', 'doing')" class="px-3 py-1.5 text-sm font-medium rounded-lg <?php echo e($statusFilter === 'doing' ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50'); ?>">Đang thực hiện</button>
            <button wire:click="$set('statusFilter', 'completed')" class="px-3 py-1.5 text-sm font-medium rounded-lg <?php echo e($statusFilter === 'completed' ? 'bg-green-50 text-green-700' : 'text-gray-600 hover:bg-gray-50'); ?>">Hoàn thành</button>
        </div>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-2 py-1.5 whitespace-nowrap text-sm font-bold text-indigo-600"><?php echo e($plan->plan_code); ?></td>
                            <td class="px-2 py-1.5 text-sm text-gray-900 font-semibold"><?php echo e($plan->asset->asset_code ?? ''); ?><br><span class="text-xs font-normal text-gray-500"><?php echo e($plan->asset->name ?? ''); ?></span></td>
                            <td class="px-2 py-1.5 text-sm text-gray-700"><?php echo e($plan->category); ?></td>
                            <td class="px-2 py-1.5 whitespace-nowrap text-sm text-gray-700"><?php echo e($plan->expected_date ? \Carbon\Carbon::parse($plan->expected_date)->format('d/m/Y') : '—'); ?></td>
                            <td class="px-2 py-1.5 whitespace-nowrap text-sm text-gray-500 text-right"><?php echo e(number_format($plan->current_odo)); ?></td>
                            <td class="px-2 py-1.5 whitespace-nowrap text-sm font-bold text-gray-900 text-right"><?php echo e(number_format($plan->maintenance_odo)); ?></td>
                            <td class="px-2 py-1.5 whitespace-nowrap text-center">
                                ...
                                <?php elseif($plan->status === 'cho_chuan_bi_vat_tu'): ?>
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-orange-100 text-orange-800">Chờ VT</span>
                                <?php elseif($plan->status === 'thieu_vat_tu'): ?>
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Thiếu VT</span>
                                <?php elseif($plan->status === 'san_sang_xuat'): ?>
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Sẵn sàng xuất</span>
                                <?php elseif($plan->status === 'dang_bao_duong'): ?>
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">Đang bảo dưỡng</span>
                                <?php elseif($plan->status === 'completed' || $plan->status === 'hoan_thanh'): ?>
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Hoàn thành</span>
                                <?php else: ?>
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800"><?php echo e($plan->status); ?></span>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </td>
                            <td class="px-2 py-1.5 whitespace-nowrap text-right text-sm font-medium">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($plan->status, ['cho_chuan_bi_vat_tu', 'pending', 'thieu_vat_tu'])): ?>
                                    <button wire:click="checkInventory(<?php echo e($plan->id); ?>)" class="text-white bg-blue-600 hover:bg-blue-700 px-1.5 py-1 text-[11px].5 rounded mr-1" title="Kiểm tra tồn kho vật tư">🔍 Kiểm tra kho</button>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($plan->status === 'san_sang_xuat'): ?>
                                    <button wire:click="createStockOut(<?php echo e($plan->id); ?>)" class="text-white bg-green-600 hover:bg-green-700 px-1.5 py-1 text-[11px].5 rounded mr-1" title="Tạo phiếu xuất kho">📦 Xuất kho</button>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                <button wire:click="edit(<?php echo e($plan->id); ?>)" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 p-1.5 rounded mr-1" title="Sửa">✏️</button>
                                <button x-on:click="if(confirm('Xóa kế hoạch này?')) $wire.delete(<?php echo e($plan->id); ?>)" class="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 p-1.5 rounded" title="Xóa">🗑️</button>
                            </td>
                        </tr>
                    ...
                    <form wire:submit.prevent="save">
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-2 sm:pb-4">
                            <h3 class="text-lg leading-6 font-bold text-gray-900 mb-4" id="modal-title">
                                <?php echo e($planId ? 'Cập nhật kế hoạch bảo dưỡng' : 'Thêm mới kế hoạch bảo dưỡng'); ?>
                            ...
                            <div class="space-y-4">
                                <div class="grid grid-cols-2 gap-2">
                                    <div>
                                        ...
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['plan_code'];
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                    ...
                                    <div>
                                        ...
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['expected_date'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-red-500 text-xs"><?php echo e($message); ?></span> <?php unset($message);
...
<?php /**PATH D:\Project\resources\views\livewire\warehouse\maintenance-plan-manager.blade.php ENDPATH**/ ?>
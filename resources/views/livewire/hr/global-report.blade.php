<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-gradient-to-br from-indigo-500 to-blue-600 rounded-2xl p-6 text-white shadow-lg">
            ...
            <div class="text-4xl font-black">{{ $projects->count() }}</div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col h-full">
            <div class="flex justify-between items-start mb-6">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">Mã DA: {{ $project->code ?? 'N/A' }}</p>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4 mt-auto">
                <div class="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    ...
                    <div class="text-2xl font-bold text-slate-800">{{ $stats[$project->id]['users'] ?? 0 }} <span class="text-sm font-normal text-slate-500">người</span></div>
                </div>
                <div class="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <div class="text-sm text-slate-500 font-medium mb-1">Giá trị tồn kho (Ước tính)</div>
                    <div class="text-xl font-bold text-slate-800">{{ number_format($stats[$project->id]['stock_value'] ?? 0, 0, ',', '.') }} <span class="text-sm font-normal text-slate-500">đ</span></div>
                </div>
            </div>
            
            <div class="mt-4 pt-4 border-t border-slate-100 flex justify-between items-center">
                <span class="text-sm text-slate-500">{{ $stats[$project->id]['active_orders'] ?? 0 }} đơn hàng đang xử lý</span>

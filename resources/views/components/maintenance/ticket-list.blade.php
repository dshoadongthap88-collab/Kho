            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($tickets as $ticket)
                    <tr class="hover:bg-slate-50 {{ in_array($ticket->id, $selectedTickets) ? 'bg-sky-50' : '' }}">
                        ...
                        <td class="px-2 py-1.5 whitespace-nowrap text-sm font-medium text-gray-900">{{ $ticket->maintenance_date ? \Carbon\Carbon::parse($ticket->maintenance_date)->format('d/m/Y') : $ticket->created_at->format('d/m/Y') }}</td>
                        <td class="px-2 py-1.5 whitespace-nowrap text-sm font-bold text-gray-900">{{ $ticket->asset->name ?? 'N/A' }}</td>
                        <td class="px-2 py-1.5 whitespace-nowrap text-sm text-gray-500 font-mono">{{ $ticket->asset->asset_code ?? 'N/A' }}</td>
                        <td class="px-2 py-1.5 whitespace-nowrap text-sm font-semibold text-gray-700">{{ $ticket->maintenance_rule_id ?? 'N/A' }}</td>
                        <td class="px-2 py-1.5 whitespace-nowrap text-sm text-gray-600">{{ $ticket->staff_name ?? 'N/A' }}</td>

                ...
                @forelse($orders as $order)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3">
                            @switch($order->status)
                                @case('pending')
                                    <span class="bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-xs">Chờ xác nhận</span>
                                    @break
                                @case('confirmed')
                                    <span class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs">Đã xác nhận</span>
                                    @break
                                @case('received')
                        ...
                        <td class="px-4 py-3 text-right flex gap-2 justify-end">
                            @if($order->status === 'pending')
                                <button wire:click="confirmOrder({{ $order->id }})" class="bg-green-500 hover:bg-green-600 text-white px-2 py-1 rounded text-xs" title="Xác nhận">✓ Xác nhận</button>
                            @endif
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-middle bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full max-h-[90vh] overflow-y-auto">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">{{ $isEdit ? 'Chỉnh sửa đơn hàng' : 'Tạo đơn hàng mới' }}</h3>
                        
                        <div class="space-y-4">
                            <!-- Basic Info -->
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Số PO</label>
                                        ...
                                        <table class="w-full text-xs">
                                            <thead>
                                                <tr class="bg-gray-100 border-b">
                                                    ...
                                                    <th class="px-2 py-2 text-right">SL</th>
                                            ...
                                            <tbody class="divide-y">
                                                @foreach($items as $index => $item)
                                                    ...
                                                    <tr class="hover:bg-gray-100">
                                                        <td class="px-2 py-2">{{ $product?->name ?? 'N/A' }}</td>
                                                        <td class="px-2 py-2 text-right">{{ $item['quantity'] }}</td>
                                                        <td class="px-2 py-2 text-right">{{ number_format($item['unit_price'], 0, ',', '.') }}</td>
                                                        <td class="px-2 py-2 text-right font-semibold">{{ number_format($item['line_total'], 0, ',', '.') }}</td>

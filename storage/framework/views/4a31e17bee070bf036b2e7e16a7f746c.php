<div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8" x-data="{ 
            barData: <?php if ((object) ('barData') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('barData'->value()); ?>')<?php echo e('barData'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('barData'); ?>')<?php endif; ?>,
            pieData: <?php if ((object) ('pieData') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('pieData'->value()); ?>')<?php echo e('pieData'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('pieData'); ?>')<?php endif; ?>,
            paretoData: <?php if ((object) ('paretoData') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('paretoData'->value()); ?>')<?php echo e('paretoData'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('paretoData'); ?>')<?php endif; ?>,
            heatMapData: <?php if ((object) ('heatMapData') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('heatMapData'->value()); ?>')<?php echo e('heatMapData'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('heatMapData'); ?>')<?php endif; ?>,
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100 italic-rows">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-4 py-3 text-xs text-gray-400 font-mono"><?php echo e($tx->created_at->format('d/m/Y H:i')); ?></td>
                    <td class="px-4 py-3">
                        <div class="text-sm font-semibold text-gray-800"><?php echo e($tx->product->name ?? ''); ?></div>
                        <div class="text-[10px] text-gray-400 font-mono"><?php echo e($tx->product->code ?? ''); ?></div>
                    </td>
                    ...
                    <td class="px-4 py-3 text-center text-sm font-black <?php echo e($tx->quantity >= 0 ? 'text-green-600' : 'text-red-600'); ?>">
                        <?php echo e($tx->quantity >= 0 ? '+' : ''); ?><?php echo e(number_format($tx->quantity)); ?>

                    </td>
                    <td class="px-4 py-3 text-xs font-medium text-gray-600 italic">👤 <?php echo e($tx->creator->name ?? '-'); ?></td>
                    <td class="px-4 py-3 text-[10px] text-gray-400 leading-tight"><?php echo e(Str::limit($tx->note, 50)); ?></td>
                </tr>
...
<?php /**PATH D:\Project\resources\views\livewire\warehouse\reports.blade.php ENDPATH**/ ?>